# app.R
# ============================================================================ #
# GCW1 — Dark Medical Theme + D3 Interactive Tree (Standalone)
# ============================================================================ #
library(shiny)
library(bslib)
library(dplyr)
...
# Base seed and splits for standard references
set.seed(123)
idx0 <- which(heart$target == "0")
idx1 <- which(heart$target == "1")
train_index <- c(
  sample(idx0, floor(0.70 * length(idx0))),
...
var_info <- list(
  ...
  sex = list(
    ...
    icon = "👤",
    unit = "(0=F,1=M)",
    desc = "Patient's biological sex."
  ),
  cp = list(
    label = "Chest Pain Type",
    icon = "💢",
    unit = "(0-3)",
    desc = "0=typical angina, 1=atypical, 2=non-anginal, 3=asymptomatic."
  ),
  trestbps = list(
    label = "Resting Blood Pressure",
    icon = "🩺",
    unit = "mmHg",
    desc = "Blood pressure at rest."
  ),
  chol = list(
    label = "Cholesterol",
    icon = "🧪",
  ...
  restecg = list(
    ...
    icon = "📈",
    unit = "(0-2)",
    desc = "0=normal, 1=ST-T abnormality, 2=LV hypertrophy."
  ),
  thalach = list(
    label = "Max Heart Rate",
    icon = "❤️",
    unit = "bpm",
    desc = "Max heart rate achieved during exercise."
  ),
  exang = list(
    label = "Exercise Angina",
    icon = "🏃",
    unit = "(0/1)",
    desc = "Exercise-induced chest pain (1=yes)."
  ),
  oldpeak = list(
    label = "ST Depression",
    icon = "📉",
  ...
  ca = list(
    ...
    icon = "🔬",
    unit = "(0-3)",
    desc = "Number of major vessels coloured by fluoroscopy."
  ),
  thal = list(
    label = "Thalassemia",
    icon = "🧬",
    unit = "(0-3)",
    desc = "Blood disorder test result."
  )
)
...
rpart_to_nodelist <- function(tree) {
  ...
  if (!is.null(splits_m) && length(non_leaf) > 0) {
    ...
    for (i in seq_len(n_map)) {
      node_thresh[[as.character(non_leaf[i])]] <- as.numeric(splits_m[
        i,
        "index"
      ])
    }
  ...
  lapply(nodes_r, function(nd) {
    ...
    explanation <- if (is_leaf) {
      paste0(
        "Based on all decisions above, these <strong>",
        n_obs,
        " patients</strong> are predicted to have <strong>",
        majority,
        "</strong>."
      )
    } else {
    ...
    list(
      ...
      icon = icon,
      unit = unit,
      threshold = if (is.null(thresh) || is.na(thresh)) {
        NULL
      } else {
        as.numeric(round(thresh, 2))
      },
      isLeaf = is_leaf,
      majority = majority,
      n = n_obs,
...
make_d3_script <- function(cid, pid, msg_data, msg_go, msg_path = NULL) {
  path_handler <- if (!is.null(msg_path)) {
    paste0(
      "
  var pathIds = new Set();
...
",
  ...
  Shiny.addCustomMessageHandler('",
    ...
    Shiny.setInputValue('",
    msg_data,
    "_max',WQ.length-1,{priority:'event'});
    draw();
  });
  Shiny.addCustomMessageHandler('",
    msg_go,
    "',function(s){ WS=s; draw(); });
...
ui <- page_sidebar(
  title = "Clinical Decision Tree: Predicting Heart Disease",
  theme = theme_gc,
  fillable = TRUE,
  ...
  sidebar = sidebar(
    ...
    tags$p(
      "🧬",
      tags$b(" Patient Profile"),
      style = "color:#57c5e0;font-size:0.95rem;"
    ),
    tags$div(
      class = "step-info-box",
      p(
        "Enter a patient's clinical values below, then click Predict to see which path through the tree they follow."
      )
    ),
    tags$br(),
    # Demographics
    tags$p(
      style = "font-size:0.75rem;color:#57c5e0;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;",
      "Demographics"
    ),
    sliderInput(
      "p_age",
  ...
  card(
    full_screen = TRUE,
    card_header(
      "🏥 Full Clinical Decision Tree — click any node to learn more"
    ),
    tags$div(class = "d3tc", id = "s4-tc", make_info_panel("s4-ip"), ecg_svg),
    tags$script(make_d3_script("s4-tc", "s4-ip", "s4td", "s4go", "s4path")),
    tags$p(
      style = "font-size:0.9em;color:#7ec8de;text-align:center;margin-top:10px;",
      tags$i(
        "Built on Training Data — accuracy scores from unseen Testing Data."
      )
    )
  )
)
...
server <- function(input, output, session) {
  output$s4_mode_desc <- renderUI({
    ...
    if (is_auto) {
      tags$div(
        ...
        tags$p(
          style = "font-size:0.75rem;color:#7ec8de;margin:0;",
  ...
  s4_data <- reactive({
    is_manual <- !is.null(input$s4_prune_mode) &&
      input$s4_prune_mode == "manual"
    split_ratio <- if (is_manual && !is.null(input$s4_split)) {
      input$s4_split
    } else {
      0.7
    }
    ...
    idx1 <- which(heart$target == "1")
    tr_idx <- c(
      sample(idx0, floor(split_ratio * length(idx0))),
    ...
    list(
      ...
      n_test = nrow(heart) - length(tr_idx),
      pct_train = round(split_ratio * 100),
      pct_test = round((1 - split_ratio) * 100)
    )
  })
    ...
    tags$div(
      ...
      tags$div(
        style = paste0(
          "flex:",
          d$pct_train,
          ";background:#1a5f7a;color:#dff0f8;padding:5px 8px;text-align:center;"
        ),
      ...
      tags$div(
        style = paste0(
          "flex:",
          d$pct_test,
          ";background:#2e0a0a;color:#f87171;padding:5px 8px;text-align:center;"
        ),
  ...
  observeEvent(
    full_tree_model(),
    {
      session$sendCustomMessage(
        "s4td",
        list(nodes = rpart_to_nodelist(full_tree_model()))
      )
  ...
  output$tab4_metrics_ui <- renderUI({
    ...
    metric_label <- function(txt, tip) {
      tags$div(
        style = "font-size:0.72rem;color:#7ec8de;text-transform:uppercase;letter-spacing:0.5px;display:flex;align-items:center;gap:4px;",
        txt,
        tooltip(
          trigger = bsicons::bs_icon(
            "info-circle",
            size = "0.85em",
            style = "color:#57c5e0;cursor:pointer;"
          ),
          tip
        )
      )
    ...
    tags$div(
      ...
      tags$div(
        style = "flex:1;padding:10px 16px;border-right:1px solid #1e3f5c;",
        metric_label(
          "Real-World Accuracy",
          "The percentage of unseen test patients the tree correctly classified. Unlike training accuracy, this reflects true performance on new data the tree has never seen before."
        ),
        tags$div(
        ...
        tags$div(
          style = "font-size:0.72rem;color:#64748b;",
      ...
      tags$div(
        ...
        metric_label(
          "Sensitivity",
          "Of all patients who actually had heart disease, what % did the tree correctly catch? This is critical in clinical settings — a low sensitivity means real cases are being missed."
        ),
        tags$div(
          style = "font-size:1.5rem;font-weight:700;color:#f87171;",
          paste0(round(sens * 100, 1), "%")
        ),
        tags$div(
          style = "font-size:0.72rem;color:#64748b;",
          "heart disease correctly caught"
        )
      ),
      tags$div(
        ...
        metric_label(
          "Specificity",
          "Of all patients who were actually healthy, what % did the tree correctly clear? Low specificity means healthy patients are being unnecessarily flagged as sick."
        ),
        tags$div(
          style = "font-size:1.5rem;font-weight:700;color:#57c5e0;",
    ...
    leaf_id <- tryCatch(
      {
        ...
        if (length(leaf_idx) > 0) {
          as.integer(rownames(m$frame)[leaf_idx[1]])
        } else {
          NULL
        }
      },
  ...
  output$predicted_condition_ui <- renderUI({
    ...
    tags$div(
      ...
      tags$div(
        style = "font-size:0.85rem;color:#7ec8de;",
        "Predicted condition:"
      ),
